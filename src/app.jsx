...
import { Home, Users, Building2, DollarSign, Wrench, Search, Plus, AlertCircle, CheckCircle, Clock, X, Sun, Moon, Settings, Menu } from 'lucide-react';

const API_BASE = import.meta.env.VITE_API_URL || 'http://localhost:4000';
const deleteButtonClass =
  'inline-flex items-center justify-center px-1.4 py-0.3 text-xs leading-tight rounded-md bg-gray-800 text-white hover:bg-gray-900 hover:shadow-sm transition-colors duration-150 dark:bg-slate-700 dark:hover:bg-slate-600';
const maintenanceDeleteButtonClass = `${deleteButtonClass} px-1.5 py-0.5 text-[11px]`;
      ...
      <div className={`${cardBg} rounded-lg shadow`}>
        ...
        <div className="space-y-3 lg:hidden">
          {payments.map(payment => (
            <div key={payment.payment_id} className="border border-gray-200 rounded-lg p-4 relative">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700 text-sm">
              {payments.map(payment => (
                <tr key={payment.payment_id} className={`${isDark ? 'hover:bg-slate-800' : 'hover:bg-slate-100'}`}>
                  ...
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{payment.paid_at ? payment.paid_at.split('T')[0] : '-'}</td>
                  <td className="px-4 py-3">
                    <div className="flex justify-end">
  ...
  const Maintenance = () => (
    <div>
      ...
      <div className="space-y-4">
        {maintenance.map(req => (
          <div key={req.request_id} className={`${cardBg} rounded-lg shadow p-6`}>
          ...
          >
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors shrink-0
                bg-transparent border border-transparent text-slate-700 hover:bg-slate-200
